...
mutationeditor <- function(){
  ...
  for(i in 1:length(targets$mutation)){
    ...
    sequence <- read.fasta(paste("~/Documents/PhD/Year1/MiniProject_1/Protein_fastas/Susceptible/",proteinqueryfolder,"/", targets$whitelisted_taxa[i], '.fasta',sep = ''),
    ...
    if(substr(sequence,start=coordinate,stop=coordinate) != original){   ###IF there are two identical adjacent bases this will mutate the wrong one!!!
      ...
      write.fasta(sequences = mutantsequence, names = proteinqueryfolder, file.out = paste("~/Documents/PhD/Year1/MiniProject_1/Protein_fastas/Resistant/" ,proteinqueryfolder, '/', targets$mutation[i],'_', species, '.fasta', sep = ''))
    }
  }
  print('Finished')
}
